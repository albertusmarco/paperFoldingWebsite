  ...
  <section class="features18 popup-btn-cards cid-rbljldAFZH" id="features18-k">
    <form id="formAdmin" name="form1" method="post" action="">
      ...
      <table align="center" width="80%" border="2">
        ...
        <?php
          $no = 0;
          $sql = "SELECT `order`.order_time, `order`.id_order, `order`.id_customer, `order_detail`.path_design, `order`.status FROM `order`, `order_detail` WHERE `order`.id_order = `order_detail`.id_order AND `order`.status != 'Cancel';";
          $data = viewData($sql);
          while ($row = mysqli_fetch_row($data))
          {
            ...
            echo ("</td><td>");
            echo '<input class="button button2" type="submit" name="Update" id="Update" value="Update" formaction="admin.php?stats='.$stat.'&orderID='.$row[1].'"/>';
            echo "</td></tr>";
          }
        ?>
      </table>
